...
class Game(Frame):
    def __init__(self):
        ...
        self.commands={c.KEY_UP:main.move_up,c.KEY_DOWN:main.move_down,c.KEY_LEFT:main.move_left,c.KEY_RIGHT:main.move_right}
        self.grid_cells=[]
        self.init_grid()
        self.init_matrix()
    ...
    def init_matrix(self):
        self.matrix=main.start_game()
        main.add_new_2(self.matrix)
        main.add_new_2(self.matrix)
    ...
    def key_down(self,event):
        ...
        if key in self.commands:
            ...
            if changed:
                ...
                self.update_grid_cells()
                changed=False
                if main.get_curr_state(self.matrix)=='WON':
                    self.grid_cells[1][1].configure(text="You",bg=c.BACKGROUND_COLOR_CELL_EMPTY)
                    self.grid_cells[1][2].configure(text="Win",bg=c.BACKGROUND_COLOR_CELL_EMPTY)
                if main.get_curr_state(self.matrix)=="LOST":
                    self.grid_cells[1][1].configure(text="You",bg=c.BACKGROUND_COLOR_CELL_EMPTY)
